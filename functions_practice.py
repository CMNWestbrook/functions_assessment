...
###############################################################################
#18 doctest failures as of this point
...
def repeat_string(any_string, any_integer):
    print any_string * int(any_integer)
...
def is_divisible_by_three(some_integer):
    if int(some_integer) % 3 == 0:
        return True
    else:
        return False
...
# for this I had to google what to do if there's only one argument passed into
# a function that takes two arguments


def total_meal_price(meal_price, tip_percentage=0.15):
    meal_total = float(meal_price) + (float(meal_price) * float(tip_percentage))
    return meal_total
...
def sign_and_parity(an_integer):
    sign_and_parity_list = []
    if int(an_integer) % 2 == 0:
...
def write_letter(name, job_title, sender_name):
    ...
    print "Dear " + title + ", I think you are amazing! Sincerely, " + sender_name
